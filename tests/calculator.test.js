...
import { calcMenu, getUnitPrice, getUnitCost } from '../js/calculator.js';
...
describe('calcMenu', () => {
  ...
  it('calculates rawCost from ingredients correctly', () => {
    const r = calcMenu({
      ingredients: [
        { totalQty: 1000, totalPrice: 550, usedQty: 17 },
      ],
      packaging: [], hiddenCosts: [],
    });
  ...
  it('applies packagingWaste correctly', () => {
    const r = calcMenu({
      ...
      packaging: [{ totalQty: 50, totalPrice: 60, usedQty: 1 }],
      packagingWaste: 10,
    });
    const raw = 60 / 50;
  ...
  it('calculates c1/c2 prices with VAT for Iced Latte', () => {
    ...
    expect(r.c1IncVAT).toBeCloseTo(r.totalDeliveryCost * 1.5 * 1.07, 4);
    expect(r.c2IncVAT).toBeCloseTo(r.totalDeliveryCost * 1.25 * 1.07, 4);
  });
  ...
  it('uses DEFAULTS when menu omits optional fields', () => {
    const r1 = calcMenu({ ingredients: [{ totalQty: 100, totalPrice: 100, usedQty: 10 }] });
    const r2 = calcMenu({
      ingredients: [{ totalQty: 100, totalPrice: 100, usedQty: 10 }],
