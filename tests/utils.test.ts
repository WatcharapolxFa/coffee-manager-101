...
import { thb, todayStr, daysInMonth, genId, menuIcon, escapeHtml } from '../src/utils.ts';

describe('thb', () => {
  it('formats zero',           () => expect(thb(0)).toBe('฿0'));
  it('formats integers',       () => expect(thb(100)).toBe('฿100'));
  it('rounds up decimals',     () => expect(thb(9.1)).toBe('฿10'));
  it('rounds 9.5 up to 10',   () => expect(thb(9.5)).toBe('฿10'));
  it('adds thousands comma',   () => expect(thb(1235)).toBe('฿1,235'));
  it('rounds up 1234.5',       () => expect(thb(1234.5)).toBe('฿1,235'));
  it('handles null',           () => expect(thb(null)).toBe('฿0'));
  it('handles undefined',      () => expect(thb(undefined)).toBe('฿0'));
  it('handles large numbers',  () => expect(thb(1000000)).toBe('฿1,000,000'));
});
...
describe('escapeHtml', () => {
  it('escapes < and >', ()  => expect(escapeHtml('<b>bold</b>')).toBe('&lt;b&gt;bold&lt;/b&gt;'));
  ...
  it('handles number input',() => expect(escapeHtml(42)).toBe('42'));
  it('handles null',         () => expect(escapeHtml(null)).toBe(''));
  it('leaves safe text alone',() => expect(escapeHtml('Hello World')).toBe('Hello World'));
});
