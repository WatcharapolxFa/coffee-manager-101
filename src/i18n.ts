...
let _lang: Lang = (localStorage.getItem('app_lang') as Lang) || 'th';
...
export function applyStatic(): void { _applyStatic(); }

function _applyStatic(): void {
  ...
  document.querySelectorAll<HTMLElement>('.lang-btn').forEach(btn => {
    btn.classList.toggle('active', (btn as HTMLButtonElement).dataset['lang'] === _lang);
  });
}
